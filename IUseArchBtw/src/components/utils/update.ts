...
async function check_for_updates({ manifest }: Props) {
  ...
  const url = `${manifest["src"]}?${Math.floor(Math.random() * 1001)}.js`;
  ...
  if (!external_version || !external_build)
  ...
  if (external_version != manifest["version"])
    return show_update_dialog(
      url,
      external_version,
      external_build.split("-")[1],
      manifest,
      false
    );
  if (external_build != manifest["build"])
    return show_update_dialog(
      url,
      external_version,
      external_build.split("-")[1],
 ...
 */
const show_update_dialog = (
  url: string,
  version: string,
  build: string,
  manifest: object,
  is_ghost_patch: boolean
) => {
  // open a dialog to show that a new version is available
  const type = is_ghost_patch ? build : version;
  Dialog.show({
    title: "Update found",
    body: `A newer ${is_ghost_patch ? "build" : "version"} is available for ${
      manifest["name"]
    }. ${
      is_ghost_patch
      ...
      is_ghost_patch ? `build \`${build}\`` : `version \`${version}\``
    }  now?`,
    confirmText: "Update",
    cancelText: "Not now",
...
const no_updates = (name: string, type: string) => {
  ...
  Toasts.open({
    content: `${name} is on latest version (${type})`,
  ...
  window.enmity.plugins.installPlugin(url, ({ data }) => {
    ...
    data == "installed_plugin" || data == "overridden_plugin"
      ? Dialog.show({
          ...
          body: `Successfully updated to ${
            is_ghost_patch ? `build` : `version`
