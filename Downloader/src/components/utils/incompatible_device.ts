...
async function check_if_compatible_device(manifest: object) {
  ...
  if (device.includes("iPhone")) {
    ...
    if (
      ...
      Storage.getItem(`__${manifest["name"]}_incompatible_dialog__`).then(
        (res: any) => {
          // opens a dialog showing the message that the iPhone model in question may cause issues.
          res ??
            Dialog.show({
              title: "Incompatible iPhone",
              body: `Please note that you're on an${
                devices[Native.DCDDeviceManager.device]
              }.\nSome features in ${
                manifest["name"]
              ...
              onConfirm: () =>
                Storage.setItem(
                  `__${manifest["name"]}_incompatible_dialog__`,
                  "true"
                ),
            });
        }
